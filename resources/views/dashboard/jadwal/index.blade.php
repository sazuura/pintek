    ...
    <main class="w-full pt-9 px-6 pb-9 font-sans max-h-[calc(100vh-56px)] overflow-y-auto overflow-x-hidden">
        ...
        <div class="bg-surface dark:bg-surface-dark rounded-xl shadow-card overflow-hidden max-xs:hidden">
            <div class="py-4 px-5 flex items-center justify-between border-b border-page-bg dark:border-page-bg-dark">
                <h3 class="text-[15px] font-semibold text-text dark:text-text-dark">{{ $bisaUbah ? 'Daftar Jadwal' : 'Daftar Jadwal yang Ditugaskan' }}</h3>
                                ...
                                <td colspan="{{ $kolomTotal }}" class="!p-0">
                                    <div class="flex flex-wrap items-start gap-x-8 gap-y-2.5 py-[18px] px-4">
                                        ...
                                        <div class="flex items-start gap-2.5 min-w-0 flex-[2_1_260px]">
                                            ...
                                            <div>
                                                <label class="text-[11px] text-text-muted uppercase tracking-[0.4px] block mb-0.5">Keterangan{{ $bisaUbah ? '' : ' / Link' }}</label>
                                        ...
                                        <div class="flex items-start gap-2.5 min-w-0 flex-[2_1_260px]">
                                            ...
                                            <div>
                                                <label class="text-[11px] text-text-muted uppercase tracking-[0.4px] block mb-0.5">Operator{{ $bisaUbah ? '' : ' Bertugas' }}</label>
        ...
        <div class="hidden max-xs:flex flex-col gap-3 mb-4">
            @forelse($jadwal as $j)
                ...
                <div class="mobile-card bg-surface dark:bg-surface-dark rounded-xl p-4 shadow-card flex flex-col gap-3.5 {{ $bisaUbah ? '' : 'cursor-pointer' }}"
                     {{ $bisaUbah ? '' : 'data-open-jadwal-modal' }}
                     ...
                     @endif>
                    <div class="flex items-center gap-3 {{ $bisaUbah ? 'cursor-pointer' : '' }}" @if($bisaUbah) data-open-jadwal-modal @endif>
                        @unless($bisaUbah)
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="font-semibold text-sm text-text dark:text-text-dark {{ $bisaUbah ? '' : 'truncate' }}">{{ $j->judul_kegiatan }}</div>
                            <div class="text-xs text-text-muted whitespace-nowrap overflow-hidden text-ellipsis">{{ $bisaUbah ? $j->operators->count() . ' operator' : ($j->keterangan ?: '-') }}</div>
                        </div>
                    ...
                    <div class="flex flex-col gap-2 pt-3 border-t border-page-bg dark:border-page-bg-dark">
                        <div class="flex items-center justify-between text-[13px] text-text-muted">
                            ...
                            <span class="{{ $bisaUbah ? '' : 'text-text dark:text-text-dark font-medium' }}">{{ $j->tanggal->translatedFormat('D, d M Y') }}</span>
                        </div>
                        <div class="flex items-center justify-between text-[13px] text-text-muted">
