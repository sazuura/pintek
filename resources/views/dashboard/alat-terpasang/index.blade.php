...
@section('content')
    @php
        ...
        $bisaTambah = auth()->user()->punyaAkses('alat-terpasang', 'tambah');
        $bisaUbah   = auth()->user()->punyaAkses('alat-terpasang', 'ubah');
        $bisaHapus  = auth()->user()->punyaAkses('alat-terpasang', 'hapus');
    @endphp
    <main class="w-full pt-9 px-6 pb-9 font-sans max-h-[calc(100vh-56px)] overflow-y-auto overflow-x-hidden">
        <div class="flex items-center justify-between gap-4 flex-wrap mb-5">
            ...
            @if($bisaTambah)
                <a href="{{ route($roleAktif . '.alat-terpasang.create') }}"
        ...
        @if($ringkasan->total() > 0)
            <div class="bg-surface dark:bg-surface-dark rounded-xl shadow-card p-4 mb-4">
                ...
                <x-pagination :paginator="$ringkasan" label="jenis alat" />
            </div>
        @endif
        ...
        <div class="bg-surface dark:bg-surface-dark rounded-[10px] py-3.5 px-4 mb-4 flex items-center gap-2.5 flex-wrap shadow-[0_2px_8px_rgba(0,0,0,0.04)] max-md:flex-col max-md:items-stretch">
            <form method="GET" action="{{ route($roleAktif . '.alat-terpasang.index') }}" class="contents">
                ...
                <select name="urutkan" onchange="this.form.submit()" data-placeholder="-- Urutkan --"
                    ...
                    <option value="jumlah_desc" {{ request('urutkan') == 'jumlah_desc' ? 'selected' : '' }}>Jumlah Terpasang Terbanyak</option>
                    <option value="jumlah_asc" {{ request('urutkan') == 'jumlah_asc' ? 'selected' : '' }}>Jumlah Terpasang Tersedikit</option>
                </select>
                @if(request()->hasAny(['search', 'tanggal', 'kondisi', 'urutkan']))
                    <a href="{{ route($roleAktif . '.alat-terpasang.index') }}"
                    ...
                    <div class="bg-surface dark:bg-surface-dark rounded-xl overflow-hidden shadow-card transition-[transform,box-shadow] duration-200 flex flex-col hover:-translate-y-[3px] hover:shadow-[0_6px_20px_rgba(0,0,0,0.10)]">
                        ...
                        <div class="p-3.5 flex flex-col gap-2 flex-1">
                            <div class="text-sm font-semibold text-text dark:text-text-dark leading-tight flex items-center gap-1.5">
                                {{ $item->nama_alat }}
                                @if($item->jumlah > 1)
                                    <span class="text-[11px] font-semibold px-1.5 py-0.5 rounded-full bg-primary/10 text-primary shrink-0">×{{ $item->jumlah }}</span>
                                @endif
                            </div>
                            ...
                            @if($item->jumlah > 1)
                                <div class="text-[13px] text-text dark:text-text-dark flex items-center justify-between">
                                    <span class="text-[13px] text-text-muted">Jumlah terpasang</span>
                                    <span class="font-medium text-text dark:text-text-dark">{{ $item->jumlah }} unit</span>
                                </div>
                            @endif
                        ...
                        @if($bisaUbah || $bisaHapus)
                            <div class="py-2.5 px-3.5 border-t border-page-bg dark:border-page-bg-dark flex gap-1.5">
                                @if($bisaUbah)
                                    <a href="{{ route($roleAktif . '.alat-terpasang.edit', $item->id_alat_terpasang) }}"
