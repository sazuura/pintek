...
@section('content')
    <main class="w-full pt-9 px-6 pb-9 font-sans max-h-[calc(100vh-56px)] overflow-y-auto overflow-x-hidden">
        <div class="flex items-center justify-between gap-4 flex-wrap mb-5">
            ...
            <a href="{{ route(auth()->user()->role . '.alat-terpasang.index') }}"
        ...
        @if($jumlahDalamKelompok > 1)
            <div class="bg-primary/10 text-primary rounded-xl p-4 mb-5 text-[13px] flex items-center gap-2">
                <i class="bx bx-info-circle text-base"></i>
                Alat ini terpasang sebanyak <strong>{{ $jumlahDalamKelompok }} unit</strong> yang identik. Perubahan yang kamu simpan akan berlaku untuk seluruh {{ $jumlahDalamKelompok }} unit tersebut.
            </div>
        @endif

        <form action="{{ route(auth()->user()->role . '.alat-terpasang.update', $alat->id_alat_terpasang) }}" method="POST"
