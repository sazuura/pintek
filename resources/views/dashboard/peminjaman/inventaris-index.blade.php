        ...
        <div class="bg-surface dark:bg-surface-dark rounded-[10px] py-3.5 px-4 mb-4 flex items-center gap-2.5 flex-wrap shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
            <form method="GET" action="{{ route('inventaris.peminjaman.index') }}" class="contents">
                <div class="relative flex-1 min-w-[180px] max-w-[300px]">
                    ...
                    <input type="text" name="search" data-live-search="#hasil-peminjaman-inventaris" autocomplete="off"
                        placeholder="Cari pemohon, keperluan, alat..." value="{{ request('search') }}"
                ...
                <input type="date" name="end" value="{{ request('end') }}" onchange="this.form.submit()"
                    class="h-9 px-2.5 border border-page-bg dark:border-page-bg-dark rounded-lg bg-page-bg dark:bg-page-bg-dark text-text dark:text-text-dark text-[13px] font-sans cursor-pointer">
                @if(request()->hasAny(['search', 'status', 'id_user', 'start', 'end']))
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr>
                            ...
                            <th class="max-md:hidden py-3 px-4 text-[11px] uppercase tracking-[0.5px] text-text-muted text-left bg-page-bg dark:bg-page-bg-dark border-b border-page-bg dark:border-page-bg-dark whitespace-nowrap">Tgl Kembali Aktual</th>
